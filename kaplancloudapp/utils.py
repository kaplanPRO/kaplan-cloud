...
def get_source_file_path(instance, filename):
    return Path(
        instance.project.directory, instance.project.source_language.iso_code, filename
    )


def get_target_file_path(instance, filename):
    return Path(
        instance.project.directory, instance.project.target_language.iso_code, filename
    )


def trim_segment(source_or_target_segment):
    source_or_target_segment = etree.tostring(
        source_or_target_segment, encoding="UTF-8"
    ).decode()
