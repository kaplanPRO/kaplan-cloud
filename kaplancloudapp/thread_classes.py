...
class CreateTargetBilingualFileThread(threading.Thread):
    ...
    def run(self):
        ...
        relevant_segments = apps.get_model("kaplancloudapp", "Segment").objects.filter(
        ...
        for relevant_segment in relevant_segments:
            ...
            bf.update_segment(
                "<target>" + relevant_segment.target + "</target>",
...
class GenerateTargetTranslationThread(threading.Thread):
    ...
    def run(self):
        ...
        if self.file_instance.source_file:
            with tempfile.TemporaryDirectory(dir=".tmp") as tmpdir:
                path_source = Path(
                    tmpdir, Path(self.file_instance.source_file.name).name
                )
                path_source.write_bytes(self.file_instance.source_file.read())

                bf.generate_target_translation(
                    self.target_dir,
...
class ImportTargetBilingualFile(threading.Thread):
    ...
    def run(self):
        project_file_segments = apps.get_model(
            "kaplancloudapp", "Segment"
        ...
        for tu in bf.gen_translation_units():
            for segment in tu:
                ...
                segment_instance.status = ("blank", "draft", "translated").index(
                    s_state
                )
...
class NewFileThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            if instance.source_file and not instance.bilingual_file:
                with tempfile.TemporaryDirectory(dir=".tmp") as tmpdir:
                    path_source = Path(
                        tmpdir, Path(self.file_instance.source_file.name).name
                    )
                    ...
                    with open(Path(tmpdir, bf.name), "rb") as bilingualfile:
                        instance.bilingual_file.save(bf.name, bilingualfile)

            elif instance.bilingual_file:
                with tempfile.TemporaryDirectory(dir=".tmp") as tmpdir:
                    path_bf = Path(
                        tmpdir, Path(self.file_instance.bilingual_file.name).name
                    )
                    path_bf.write_bytes(self.file_instance.bilingual_file.read())

                    bf = open_bilingualfile(path_bf)

            ProjPreSettingsModel = apps.get_model(
                "kaplancloudapp", "ProjectPreprocessingSettings"
            )
            TMEntryModel = apps.get_model("kaplancloudapp", "TMEntry")
            try:
                proj_pre_settings = ProjPreSettingsModel.objects.get(
                ...
                if pretranslate:
                    for tm in instance.project.translationmemories.all():
                        relevant_tm_entries = (
                            relevant_tm_entries
                            | TMEntryModel.objects.filter(translationmemory=tm)
                        )
            ...
            for xml_translation_unit in bf.gen_translation_units():
                for xml_segment in xml_translation_unit:
                    ...
                    segment_source = etree.tostring(
                        xml_segment.find("source", xml_segment.nsmap), encoding="UTF-8"
                    ).decode()
                    ...
                    if segment_target.text is None and len(segment_target) == 0:
                        ...
                        segment_target = etree.tostring(
                            segment_target, encoding="UTF-8"
                        ).decode()
                    ...
                    if segment.target == "" and pretranslate:
                        ...
                        if tm_hits != []:
                            ...
                            target_segment = KDB.entry_to_segment(
                                target_entry, "target", reversed_tags, source_segment
                            )
...
class TMImportThread(threading.Thread):
    def __init__(
        self, entries, TMEntryModel, translation_memory, relevant_tm_entries, **kwargs
    ):
