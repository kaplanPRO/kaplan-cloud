...
def prepare_target_bilingualfile(apps, schema_editor):
    ...
    for project_file in ProjectFile.objects.all():
        with tempfile.TemporaryDirectory(dir=".tmp") as tmpdir:
            path_target_bf = Path(
                tmpdir, Path(project_file.source_bilingualfile.name).name
            )
            ...
            for relevant_segment in Segment.objects.filter(file=project_file):
                target_bf.update_segment(
                    "<target>" + relevant_segment.target + "</target>",
                    relevant_segment.tu_id,
                    relevant_segment.s_id,
                    segment_state=("blank", "draft", "translated")[
                        int(relevant_segment.status)
                    ],
                )
            target_bf.save(tmpdir)

            with open(path_target_bf, "rb") as target_bf:
                project_file.target_bilingualfile.save(path_target_bf.name, target_bf)
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(remove_target_bilingualfile, prepare_target_bilingualfile),
