...
def newproject(request):
    ...
    form.fields["translation_memories"].choices = (
        (request.POST.get("translation_memories"), "-----"),
    )
    if form.is_valid():
        new_project = Project()
...
def projects(request):
    ...
    project_files = project_files.filter(status__gte=4) & project_files.filter(
        status__lte=6
    )
...
def project(request, uuid):
    ...
    if (
        ...
        project.client is not None and request.user in project.client.team.all()
    ):
        project_files = project_files.filter(
            translator=request.user
        ) | project_files.filter(reviewer=request.user)
        ...
        if request.POST.get("task") == "download_translations":
            ...
            with tempfile.TemporaryDirectory(dir=".tmp") as tmpdir:
                for pf_id in request.POST["file_uuids"].split(";"):
                    ...
                    threads.append(
                        GenerateTargetTranslationThread(project_file, tmpdir)
                    )
                ...
                tmpdir_files = [
                    tmpdir_file
                    for tmpdir_file in list(Path(tmpdir).iterdir())
                    if not tmpdir_file.is_dir()
                ]
            ...
            for file_uuid in request.POST["file_uuids"].split(";"):
                projectreport_instance.project_files.add(
                    project_files.get(uuid=file_uuid)
                )
            ...
            files_to_export = [
                id for id in request.POST["file_uuids"].split(";") if id != ""
            ]
            ...
            with tempfile.TemporaryDirectory(dir=".tmp") as p_tmpdir:
                ...
                for project_file_instance in ProjectFile.objects.filter(
                    ...
                    if project_file_instance.source_file:
                        path_source = (
                            p_s_tmpdir
                            / Path(project_file_instance.source_file.name).name
                        )
                        path_source.write_bytes(
                            project_file_instance.source_file.read()
                        )
                        file_manifest["source"] = str(path_source)
                    if project_file_instance.bilingual_file:
                        path_source_bf = (
                            p_s_tmpdir
                            / Path(project_file_instance.bilingual_file.name).name
                        )
                        path_source_bf.write_bytes(
                            project_file_instance.bilingual_file.read()
                        )
                        ...
                        threads.append(
                            CreateTargetBilingualFileThread(
                                project_file_instance, p_t_tmpdir, path_source_bf
                            )
                        )
                ...
                with tempfile.TemporaryDirectory(dir=".tmp") as package_tmpdir:
                    path_package = Path(
                        package_tmpdir, (datetime.now().isoformat() + ".kpp")
                    )

                    project_package.export(
                        target_path=str(path_package), files_to_export=files_to_export
                    )

                    project_package_instance.package.save(
                        path_package.name, path_package.open("rb")
                    )
            ...
            with tempfile.TemporaryDirectory(dir=".tmp") as tmpdir:
                with zipfile.ZipFile(form.files.getlist("package")[0]) as kpp:
                    ...
                    for i, file in manifest["files"].items():
                        project_file_instance = ProjectFile.objects.filter(
                            project=project
                        ...
                        threads.append(
                            ImportTargetBilingualFile(
                                project_file_instance, path_target_bf
                            )
                        )
            ...
            if form1.is_valid():
                for f_uuid in request.POST["file_uuids"].split(";"):
                    ...
                    if int(request.POST["role"]) == 0:
                        if (
                            project_file.translator is not None
                            and request.POST.get("override") is None
                        ):
                            continue
                        project_file.translator = get_user_model().objects.get(
                            username=request.POST["username"]
                        )
                        project_file.save()
                    elif int(request.POST["role"]) == 1:
                        if (
                            project_file.reviewer is not None
                            and request.POST.get("override") is None
                        ):
                            continue
                        project_file.reviewer = get_user_model().objects.get(
                            username=request.POST["username"]
                        )
                        project_file.save()
...
def editor(request, uuid):
    ...
    if (
        request.user.has_perm("kaplancloudapp.change_projectfile")
        ...
        or (
            ...
            and request.user in project_file.project.client.team.all()
        )
    ):
        can_edit = project_file.status < 7
    elif (
        (
            project_file.translator == request.user
            or project_file.reviewer == request.user
        )
        and project_file.status >= 4
        and project_file.status <= 6
    ):
    ...
    if request.method == "POST":
        if request.POST.get("task") == "update_segment":
            ...
            segment = (
                Segment.objects.filter(file=project_file)
            ...
            segment.status = ("blank", "draft", "translated").index(
                segment_dict["status"]
            )
            ...
            for tm in project_file.project.translationmemories.all():
                try:
                    tm_entry = TMEntry.objects.filter(translationmemory=tm).get(
                        source=source
                    )
            ...
            relevant_segment = Segment.objects.filter(file=project_file).get(
                s_id=segment_id
            )
        ...
        if request.GET.get("task") == "lookup":
            ...
            for tm in project_file.project.translationmemories.all():
                for relevant_tm_entry in TMEntry.objects.filter(translationmemory=tm):
                    tm_entry_source = KDB.entry_to_segment(
                        relevant_tm_entry.source,
                        "source",
                        reversed_tags,
                        segment_source,
                    )
                    tm_entry_source = etree.tostring(
                        tm_entry_source, encoding="UTF-8"
                    ).decode()[8:-9]

                    tm_entry_target = KDB.entry_to_segment(
                        relevant_tm_entry.target,
                        "target",
                        reversed_tags,
                        segment_source,
                    )
                    tm_entry_target = etree.tostring(
                        tm_entry_target, encoding="UTF-8"
                    ).decode()[8:-9]
                    if (
                        relevant_tm_entry.source == source_entry
                        and relevant_tm_entry.target != ""
                    ):
                        tm_entries.append(
                            (
                                1.0,
            ...
            for tm in project_file.project.translationmemories.all():
                ...
                relevant_tm_entries = relevant_tm_entries.filter(
                    source__contains=query
                ) | relevant_tm_entries.filter(target__contains=query)

                for relevant_tm_entry in relevant_tm_entries:
                    tm_entry_source = KDB.entry_to_segment(
                        relevant_tm_entry.source, "source"
                    )
                    tm_entry_source = etree.tostring(
                        tm_entry_source, encoding="UTF-8"
                    ).decode()[8:-9]
                    ...
                    tm_entry_target = KDB.entry_to_segment(
                        relevant_tm_entry.target, "target"
                    )
                    tm_entry_target = etree.tostring(
                        tm_entry_target, encoding="UTF-8"
                    ).decode()[8:-9]
                    ...
                    if query in tm_entry_source and query not in tm_entry_target:
                        ...
                        len_diff = abs(
                            len(query)
                            - ((len(tm_entry_source) + len(tm_entry_target)) / 2)
                        )
            ...
            translation_units = {}
            for segment_instance in Segment.objects.filter(file=project_file).order_by(
                "s_id"
            ):
                if segment_instance.tu_id not in translation_units:
                    translation_units[segment_instance.tu_id] = {}
                translation_units[segment_instance.tu_id][segment_instance.s_id] = (
                    segment_instance
                )
...
def reference_file(request, uuid):
    ...
    is_relevant = True
    if (
        not not request.user.has_perm("kaplancloudapp.view_projectreferencefile")
        ...
        is_relevant = False
        for project_file_instance in ProjectFile.objects.filter(
            project=reference_file_instance.project
        ):
            if (
                request.user == project_file_instance.translator
...
def translation_memories(request):
    ...
    if request.GET.get("source"):
        translation_memories = translation_memories.filter(
    ...
    if request.GET.get("target"):
        translation_memories = translation_memories.filter(
            target_language=request.GET["target"]
        )
        display_form = True
    ...
    if not request.user.has_perm("kaplancloudapp.view_translationmemory"):
        translation_memories = translation_memories.filter(
            client__in=Client.objects.filter(team=request.user)
        )
    ...
    if request.GET.get("format") == "JSON":
        ...
        return render(
            request,
            "translation-memories.html",
            {
                "at_tms": True,
                "tms": translation_memories,
                "form": form,
                "display_form": display_form,
            },
        )
...
def translation_memory(request, uuid):
    ...
    if not request.user.has_perm("kaplancloudapp.view_translationmemory") and not (
        tm.client is not None and request.user not in tm.client.team.all()
    ):
        return redirect("/accounts/login?next={0}".format(request.path))

    tm_entries = TMEntry.objects.filter(translationmemory=tm).exclude(target="")
...
def translation_memory_import(request, uuid):
    ...
    if form.is_valid():
        ...
        relevant_tm_entries = TMEntry.objects.filter(
        ...
        for entries in sliced_entries:
            new_thread = TMImportThread(
                entries, TMEntry, translation_memory, relevant_tm_entries
            )
