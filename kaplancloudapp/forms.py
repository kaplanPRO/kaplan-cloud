...
class AssignLinguistForm(forms.Form):
    ...
    role = forms.ChoiceField(
        choices=((0, "Translator"), (1, "Reviewer")),
        initial=0,
...
class ProjectForm(forms.Form):
    ...
    source_language = forms.ModelChoiceField(
        ...
        help_text="If you don't see the language you need, please create a LanguageProfile in the Admin dashboard.",
    )
    ...
    project_files = MultipleFileField(
    ...
    def clean_translation_memories(self):
        if (
            self.cleaned_data["translation_memories"] != ""
            and self.cleaned_data["translation_memories"] != "-----"
        ):
            tm = TranslationMemory.objects.get(
                id=self.cleaned_data["translation_memories"]
            )
            if (
                tm.source_language != self.cleaned_data["source_language"]
                or tm.target_language != self.cleaned_data["target_language"]
            ):
                raise ValidationError(
                    "TM language pair is different from that of the project."
                )
            else:
                return [tm]
        else:
                ...
                try:
                    ...
                    try:
                        ...
                        validationerrors.append(
                            ValidationError("{0} not compatible.".format(file.name))
                        )
...
class SearchForm(forms.Form):
    source = forms.ModelChoiceField(
        queryset=LanguageProfile.objects.all(), to_field_name="iso_code", required=False
    )
...
class TranslationMemoryImportForm(forms.Form):
    source_language = forms.CharField(
        max_length=10, required=False, help_text="Language code"
    )
    target_language = forms.CharField(
        max_length=10, required=False, help_text="Language code"
    )
    tm_file = forms.FileField(
        label="TM file",
