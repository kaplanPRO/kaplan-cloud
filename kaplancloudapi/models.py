...
class ProjectWebHook(WebHook):
    ...
    def fire_hook(self):
        return super().fire_hook(
            {"id": self.project.id, "status": self.project.get_status()}
        )


class ProjectFileWebHook(WebHook):
    project_file = models.ForeignKey(
        "kaplancloudapp.ProjectFile", on_delete=models.CASCADE
    )

    def fire_hook(self):
        return super().fire_hook(
            {"id": self.project_file.id, "status": self.project_file.get_status()}
        )
